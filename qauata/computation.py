...
from .opcodes import QAUATA_OPCODES
...
class QauataComputation(BaseComputation):
    """
    A class for all execution computations in the ``Frontier`` fork.
    Inherits from :class:`~eth.vm.computation.BaseComputation`
    """
    # Override
    opcodes = QAUATA_OPCODES
    _precompiles = QAUATA_PRECOMPILES     # type: ignore # https://github.com/python/mypy/issues/708 # noqa: E501
    ...
    def apply_keystore_message(
            ...
            message: MessageAPI,
            transaction_context: TransactionContextAPI) -> ComputationAPI:
        state.storage_address(message.sender, 0)
