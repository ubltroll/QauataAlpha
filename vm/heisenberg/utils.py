...
def calculate_intrinsic_gas(
    ...
    if transaction.to == CREATE_CONTRACT_ADDRESS:
        ...
        create_cost = 0
    if is_keystore_transaction(transaction) and gas_schedule.get('gas_tx_keystore_force'):
...
def is_keystore_transaction(transaction) -> bool:
    return (
        transaction.nonce == 0 and
        transaction.to == transaction.sender and
        transaction.value == 0 and
        data_is_keystore(transaction.sender,transaction.data)
    )
