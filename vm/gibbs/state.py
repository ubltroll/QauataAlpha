...
from .computation import GibbsComputation
...
from .utils import (
    ...
    is_gibbs_keystore_transaction,
)


class GibbsTransactionExecutor(BaseTransactionExecutor):
    def validate_transaction(self, transaction: GibbsSignedTransactionAPI) -> None:

        # Validate the transaction
        transaction.validate()
        self.vm_state.validate_transaction(transaction)

    def build_evm_message(self, transaction: GibbsSignedTransactionAPI) -> MessageAPI:
        ...
        if is_gibbs_keystore_transaction(transaction):
    ...
    def build_computation(self,
                          message: MessageAPI,
                          transaction: GibbsSignedTransactionAPI) -> ComputationAPI:
    ...
    def finalize_computation(self,
                             transaction: GibbsSignedTransactionAPI,
...
class GibbsState(BaseState):
    computation_class: Type[ComputationAPI] = GibbsComputation
    transaction_context_class: Type[TransactionContextAPI] = GibbsTransactionContext
    account_db_class: Type[AccountDatabaseAPI] = AccountDB
    transaction_executor_class: Type[TransactionExecutorAPI] = GibbsTransactionExecutor
    ...
    def apply_transaction(self, transaction: GibbsSignedTransactionAPI) -> ComputationAPI:
        executor = self.get_transaction_executor()
        return executor(transaction)

    def validate_transaction(self, transaction: GibbsSignedTransactionAPI) -> None:
        validate_gibbs_transaction(self, transaction)

    @classmethod
    def get_transaction_context(cls,
                                transaction: GibbsSignedTransactionAPI) -> GibbsTransactionContext:
        return cls.get_transaction_context_class()(
            gas_price=transaction.gas_price,
            origin=transaction.sender,
            is_keystore=is_gibbs_keystore_transaction(transaction)
        )
