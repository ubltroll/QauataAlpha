...
from .blocks import GibbsBlock
from .constants import MAX_REFUND_QUOTIENT
from .state import GibbsState
from .headers import (
    create_gibbs_header_from_parent,
    compute_gibbs_difficulty,
    configure_gibbs_header,
)
...
from .validation import (
    validate_gibbs_transaction_against_header,
    validate_gas_limit,
)


def make_gibbs_receipt(computation: ComputationAPI,
...
class GibbsVM(VM):
    # fork name
    fork: str = 'gibbs'  # noqa: E701  # flake8 bug that's fixed in 3.6.0+

    # classes
    block_class: Type[BlockAPI] = GibbsBlock
    _state_class: Type[StateAPI] = GibbsState

    # methods
    create_header_from_parent = staticmethod(create_gibbs_header_from_parent)    # type: ignore
    compute_difficulty = staticmethod(compute_gibbs_difficulty)      # type: ignore
    configure_header = configure_gibbs_header
    validate_transaction_against_header = validate_gibbs_transaction_against_header
    ...
    # Transactions
    #
    def create_transaction(self, *args: Any, **kwargs: Any) -> 'GibbsSignedTransactionAPI':
        return self.get_transaction_builder().new_transaction(*args, **kwargs)

    def create_unsigned_transaction(self, *args: Any, **kwargs: Any) -> 'GibbsUnsignedTransactionAPI':
